...
UK_SILVER_PATH = SILVER_DIR / 'off_uk_silver.csv'
...
def selective_normalize(text):
    ...
    t = str(text).lower()
    # Strip ONLY common weight/volume units (400g, 1kg, 500ml, 1.5l, etc.)
    # We leave numbers like "1%" or "2%" intact in the name
    ...
    # Clean up name but keep critical characters like %
    name = re.sub(r'[^a-zA-Z0-9% ]', ' ', name)
    name = re.sub(r'\s+', ' ', name).strip()
    return (name if name else "unknown"), (" | ".join(sizes) if sizes else "")

def round_sig_figs(val, n=2):
    if pd.isna(val) or val == 0: return val
    try:
        magnitude = np.floor(np.log10(np.abs(val)))
        decimals = n - 1 - magnitude
        return np.around(val, int(decimals))
    except: return val

def build_uk_silver():
    logger.info("=" * 60)
    logger.info("STAGE: UK SILVER REFINEMENT (Full Profile 10% Merge)")
    logger.info("=" * 60)
    ...
    # 1. UK Filtration & Pre-filtering
    logger.info("Filtering for UK products with complete macros...")
    df = df[df['countries_en'].str.contains("United Kingdom", case=False, na=False)].copy()
    initial_uk_count = len(df)
    
    df = df.rename(columns=OFF_META_MAPPING).rename(columns=OFF_MAPPING)
    core_macros = ['Calories (kcal)', 'Fat (g)', 'Protein (g)', 'Carbohydrate (g)']
    df = df.dropna(subset=[c for c in core_macros if c in df.columns])
    filtered_uk_count = len(df)
    logger.info(f"UK Products: {initial_uk_count:,} -> {filtered_uk_count:,} (with full macros)")
    ...
    logger.info("Phase 2: Full-Profile 10% Smart Merging... (Vectorized)")
    
    # To keep it fast, we group by norm_name and verify compatibility.
    # Group rows where all non-null nutrients match within 10%.
    df['nutri_count'] = df[nut_cols].notna().sum(axis=1)
    df['row_index'] = range(len(df))
    
    # Sort: Archetype first, then completeness
    ...
    def smart_merge_group(sub):
        if len(sub) <= 1: return sub.head(1).copy()
        sub = sub.copy()
        # Seed first archetype
            ...
            for arc in archetypes:
                ...
                for nut in nut_cols:
                    ...
                    if pd.notna(v1) and pd.notna(v2):
                        # 10% Relative difference check
                        diff = abs(v1 - v2)
                        mx = max(abs(v1), abs(v2), 1e-9)
                        if diff / mx > 0.10: # 10% Threshold
                            is_compat = False; break
                
                if is_compat:
                    # Match found! We will aggregate stats later, for now just label them
                    # Or we can do a rolling merge into the dict?
                    # Let's just track the archetype ID
        ...
        # We've labeled everything. Now aggregate properly.
        # This function returns a special ID for each unique profile within the name group.
        return sub

    # Apply smart merge labeling (Fast for small subsets)
    # Give every row an arc_id within its group
    df['arc_id'] = 0
    ...
    # Post-process Min/Max: set to NaN if count is 1 or all match exactly
    for c in nut_cols:
        count_col = f"{c}_count"
        min_col = f"{c}_min"
        max_col = f"{c}_max"
        if count_col in final_df.columns:
            # If all match exactly (min == max), set min/max to NaN to keep it clean
    ...
    final_df.to_csv(UK_SILVER_PATH, index=False)
    logger.info(f"DONE! UK Gold Silver archetypes: {len(final_df):,}")
    logger.info(f"Saved to {UK_SILVER_PATH}")
